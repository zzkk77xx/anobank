...
- **Capital stays productive.** Funds live in Trading Desk Treasury (yield strategies) until the moment they're needed. The Unlink spending pool holds only 10% of total deposits.
...
```
┌─────────────────────────────────────────────────────────────┐
│             TRADING DESK — TREASURY SAFE (3/5)             │
...
4. **90/10 split:** the backend automatically sweeps 90% of the deposit to the Trading Desk Treasury Safe and retains 10% in the Unlink pool as spending liquidity.
...
- **High-water mark:** when pool balance exceeds cap → sweep excess back to Trading Desk Safe via Unlink withdrawal.
...
| **TreasuryVault (xVault)** | Role-based access for Trading Desk Safe. Operator (<€10k), Manager (<€100k), Director (unlimited). Whitelist + reserve requirements. | New — AnoBank |
| **TreasuryTimelock**       | Time-delay enforcement for Trading Desk. Operations above USD threshold require configurable delay (up to 7 days).                   | New — AnoBank |
...
### 5.4 Treasury Modules (Trading Desk)
...
| **Pool Manager**  | `pool.ts`      | JIT pool funding. Periodic balance checks, low-water top-up, high-water sweep to Trading Desk. Configurable thresholds.                                                               |
...
| **SubAccount**           | Trading Desk sub-accounts: operator, balance, deployed, daily limit, protocol allowlists, PnL.                                                                           |
...
- **Liquidity Overview:** total deposits, Unlink pool balance, Trading Desk deployment percentage, idle capital
...
| Contract         | Address                                      |
| ---------------- | -------------------------------------------- |
| Trading Desk Treasury Safe | `0x8056999c18eE7f5376502630dfa7b58e799F531a` |
...
- **Treasury protection:** Trading Desk uses 3/5 multisig + role-based TreasuryVault + TreasuryTimelock for large operations. Reserve requirements prevent over-deployment.
